...
## *Part 1: Introduction*
  ...
  this assignemnet focuses on 3NF and 4NF.
...
*Third Normal Form (3NF)*
 ...
 a **non-primary key attribute**(City)  acting as a primary key for another **non-primary key attributes**
 ...
 and deleted the columns from the main table ensuring that that every non-primary key attribute is directly dependent on the 
 primary key hence reducing redundancy, minizing anomalies and promoting data integrity.
...
###Challenges
In this assignemnt, given that the entire assignment based on the data created by ourselves, I had a minor challenge in
inserting data into the PropertyUtilities table from the PropertyDetails table matching the values in PropertyID foreign key in both tables.

###Solutions
I inserted the values manually to match the PropertyID and PropertyUtilities in both the tables.
 
Conclusion
Overall, both 3NF and 4NF play crucial roles in database design by promoting data integrity, minimizing redundancy, and reducing data anomalies. 
Complying with these normalization standard ensures that databases are efficient and maintainable in the long run.
